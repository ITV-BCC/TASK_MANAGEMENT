...
export const assignTask = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const check = await pool.query(
            "SELECT 1 FROM task_assignments WHERE task_id = $1 AND employee_id = $2",
            [task_id, employee_id]
        );
        ...
        if (check.rowCount && check.rowCount > 0) {
            ...
            if (numLeft === 0) {
                ...
                await pool.query(
                    "UPDATE tasks SET status = 'CREATED', updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND status = 'ASSIGNED'",
                    [task_id]
                );
            }
